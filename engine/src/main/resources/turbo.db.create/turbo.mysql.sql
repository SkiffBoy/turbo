...
CREATE TABLE IF NOT EXISTS `em_flow_definition` (
    ...
    `flow_module_id`    varchar(128)                                            COMMENT '流程模型id',
    `flow_name`         varchar(64)                                             COMMENT '流程名称',
    `flow_key`          varchar(32)                                             COMMENT '流程业务标识',
    `tenant_id`         varchar(16)                                             COMMENT '业务方标识',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=COMPACT COMMENT='流程部署表';


DROP TABLE IF EXISTS `ei_flow_instance`;
CREATE TABLE IF NOT EXISTS `ei_flow_instance`(
    `id`                      bigint(20) unsigned NOT NULL AUTO_INCREMENT           COMMENT '自增主键',
    `flow_instance_id`        varchar(128)                                          COMMENT '流程执行实例id',
    `parent_flow_instance_id` varchar(128)                                          COMMENT '父流程执行实例id',
    `flow_deploy_id`          varchar(128)                                          COMMENT '流程模型部署id',
    `flow_module_id`          varchar(128)                                          COMMENT '流程模型id',
    `tenant_id`               varchar(16)                                           COMMENT '业务方标识',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=COMPACT COMMENT ='流程执行实例表';
...
(
    `id`                   bigint(20) unsigned NOT NULL AUTO_INCREMENT          COMMENT '自增主键',
    `flow_instance_id`     varchar(128)                                         COMMENT '流程执行实例id',
    `node_instance_id`     varchar(128)                                         COMMENT '节点执行实例id',
    `node_key`             varchar(64)                                          COMMENT '节点唯一标识',
    `sub_flow_instance_id` varchar(128)                                         COMMENT '子流程执行实例id',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=COMPACT COMMENT ='父子流程实例映射表';
...
(
    ...
    `node_instance_id`         varchar(128)                                         COMMENT '节点执行实例id',
    `flow_instance_id`         varchar(128)                                         COMMENT '流程执行实例id',
    `source_node_instance_id`  varchar(128)                                         COMMENT '上一个节点执行实例id',
    `instance_data_id`         varchar(128)                                         COMMENT '实例数据id',
    `flow_deploy_id`           varchar(128)                                         COMMENT '流程模型部署id',
    `node_key`                 varchar(64)                                          COMMENT '节点唯一标识',
    `source_node_key`          varchar(64)                                          COMMENT '上一个流程节点唯一标识',
    `tenant_id`                varchar(16)                                          COMMENT '业务方标识',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=COMPACT COMMENT='节点执行实例表';
...
CREATE TABLE IF NOT EXISTS `ei_node_instance_log` (
    ...
    `caller`                varchar(100)                                        COMMENT '调用方',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=COMPACT COMMENT='节点执行记录表';


DROP TABLE IF EXISTS `ei_instance_data`;
CREATE TABLE IF NOT EXISTS `ei_instance_data` (
    `id`                bigint(20) unsigned NOT NULL AUTO_INCREMENT             COMMENT '自增主键',
    `node_instance_id`  varchar(128)         COLLATE utf8mb4_unicode_ci         COMMENT '节点执行实例id',
    `flow_instance_id`  varchar(128)         COLLATE utf8mb4_unicode_ci         COMMENT '流程执行实例id',
    `instance_data_id`  varchar(128)         COLLATE utf8mb4_unicode_ci         COMMENT '实例数据id',
    `flow_deploy_id`    varchar(128)         COLLATE utf8mb4_unicode_ci         COMMENT '流程模型部署id',
    `flow_module_id`    varchar(128)         COLLATE utf8mb4_unicode_ci         COMMENT '流程模型id',
    `node_key`          varchar(64)          COLLATE utf8mb4_unicode_ci         COMMENT '节点唯一标识',
    `tenant_id`         varchar(16)          COLLATE utf8mb4_unicode_ci         COMMENT '业务方标识',
    ...
    `create_time`       datetime    NOT NULL DEFAULT CURRENT_TIMESTAMP          COMMENT '流程创建时间',
    `archive`           tinyint(4)  NOT NULL DEFAULT '0'                        COMMENT '归档状态(0未删除，1删除)',
    `tenant`            varchar(100)         COLLATE utf8mb4_unicode_ci         COMMENT '租户',
    `caller`            varchar(100)         COLLATE utf8mb4_unicode_ci         COMMENT '调用方',
    PRIMARY KEY (`id`),
